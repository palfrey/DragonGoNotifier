...
class DragonSyncAdapter(context: Context, autoInitialize: Boolean) : AbstractThreadedSyncAdapter(context, autoInitialize) {
    ...
    override fun onPerformSync(account: Account, extras: Bundle, authority: String, provider: ContentProviderClient, syncResult: SyncResult) {
        Log.d("dragonSync", "onPerformSync for account[" + account.name + "]")
        try {
            ...
            if (gamesToRemote.isEmpty()) {
                Log.d("dragonSync", TAG + "> No local changes to update server")
            } else {
                Log.d("dragonSync", TAG + "> Updating remote server with local changes")

                // Updating remote games
                for (remoteGame in gamesToRemote) {
                    Log.d("dragonSync", TAG + "> Local -> Remote [" + remoteGame.opponent_handle + "]")
            ...
            if (gamesToLocal.isEmpty()) {
                Log.d("dragonSync", TAG + "> No server changes to update local database")
            } else {
                Log.d("dragonSync", TAG + "> Updating local database with remote changes")
